...
object Pain001 extends ISO20022

/**
  * Holds a whole SEPA CT file.
  * Some of the fields are calculated for convenience.
  * Including:
  * MsgId = some uuid
  * CreDtTm=today
  * NbOfTxs=taken from number of Txs
  * PmtInfId = same as MsgId
  * NbOfTxs=taken from number of Txs
  * ReqdExctnDt = today
  *
  * class ISO20022DOM(name: String, iban: String, bic: String)
  * /
  */
class Pain001(account : Account, transactions : List[ISO20022Transaction]) extends ISO20022 {

  val id = UUID.randomUUID.toString

  override val routingKeyPart = "pain.001.003.03"
  /**
    *  xml represented as Elem
    */
  override val xml =
    <Document xsi:schemaLocation="urn:iso:std:iso:20022:tech:xsd:pain.001.003.03 pain.001.003.03.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.003.03">
      <CstmrCdtTrfInitn>
        <GrpHdr>
          <MsgId>{ id }</MsgId>
          <CreDtTm>{ ISODate.getCurrentDate() }</CreDtTm>
          <NbOfTxs>{ transactions.size }</NbOfTxs>
          <InitgPty>
            <Nm>{ account.name }</Nm>
          </InitgPty>
        </GrpHdr>
        <PmtInf>
          ...
          <ReqdExctnDt>{ ISODate.getCurrentDate() }</ReqdExctnDt>
          <Dbtr>
            <Nm>{ account.name }</Nm>
          </Dbtr>
          <DbtrAcct>
          ...
          { transactions.map(transfer => transfer.xml) }
        </PmtInf>
      </CstmrCdtTrfInitn>
    </Document>

  override val xsd = <xs:schema xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.002.03" xmlns:xs="http://www.w3.org/2001/XMLSchema" targetNamespace="urn:iso:std:iso:20022:tech:xsd:pain.001.002.03" elementFormDefault="qualified">
