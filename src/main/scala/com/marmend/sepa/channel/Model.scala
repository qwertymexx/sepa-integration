...
import com.marmend.sepa.channel.util.ISODate
...
object Model {
  ...
  /**
    ...
    */
  sealed abstract case class Account(name: String, iban: String, bic: String)

  object MyBankAccount extends Account("account_name", "DE12341000000039334349", "Mybank_BIC")
  ...
  trait ISO20022 {
    ...
    def fullXmlFile(): String = {
      """<?xml version="1.0" encoding="utf-8"?>""" + xml.mkString
    }

  }

  abstract class CreditTransfer() extends ISO20022

  abstract class DirectDebit() extends ISO20022

  case class Transaction (endToEndId : String,
                          instdAmt : Double,
                          fullName : String,
                          iBAN : String,
                          rmtInf : Option[String])
  object Transaction

  case class Pain001(account: Account, transactions: List[Pain001Transaction]) extends CreditTransfer {

    val id = UUID.randomUUID.toString

    val xml =
      <Document xsi:schemaLocation="urn:iso:std:iso:20022:tech:xsd:pain.001.003.03 pain.001.003.03.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.003.03">
        <CstmrCdtTrfInitn>
          <GrpHdr>
            <MsgId>
              {id}
          ...
          <PmtInf>
            ...
            </DbtrAgt>{transactions.map(transfer => transfer.xml)}
          </PmtInf>
        </CstmrCdtTrfInitn>
      </Document>

    val xsd = <xs:schema xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.002.03" xmlns:xs="http://www.w3.org/2001/XMLSchema" targetNamespace="urn:iso:std:iso:20022:tech:xsd:pain.001.002.03" elementFormDefault="qualified">
      ...
      <xs:complexType name="PaymentInstructionInformationSCT">
        <xs:sequence>
          ...
          <xs:element name="PmtTpInf" type="PaymentTypeInformationSCT1" minOccurs="0">
            <xs:annotation>
              <xs:documentation>
                If used, it is recommended to be used only at �Payment Information� level and not at Credit Transfer Transaction Information� level. When Instruction Priority is to be used, �Payment Type Information� must be present at �Payment Information� level.
              </xs:documentation>
            </xs:annotation>
          </xs:element>
  ...
  case class Pain001Transaction(transaction: Transaction) extends CreditTransfer {
